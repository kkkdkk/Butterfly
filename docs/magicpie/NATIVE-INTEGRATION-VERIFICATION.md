...
> 本文按时间追加，下面开头的状态是早期实验记录。当前推荐路线、问题速查与复用入口见 [HANDWRITING-RUNBOOK.md](HANDWRITING-RUNBOOK.md)；最新实笔、保存和跨端验收在本文末尾。不要将早期固定笔宽/逐笔交接方案当成当前实现。
...
### 2026-09-16 保存与跨端收尾（进行中）

- 真机撤销/重做产生 replay 事件，但保存文件不变；回归确认 replay 没有标记文档未保存，且 `force:true` 被 saved 早退拦截。这是保存状态错误，不是压力数据格式问题。
- 修复：有可用历史时 Undo/Redo 标记 unsaved，并按已有设置触发自动保存；保留 absoluteRead。force 仅绕过普通 saved，不绕过 absoluteRead；显式 Save As 的既有语义不变。
- 新增 5 项回归：撤销/重做标脏、强制保存、撤销/重做文件往返、只读强制保存、撤销后自动保存。全量普通测试 136 项通过；相关 22 项通过；analyze 无问题。默认延迟自动保存仍遵守既有延迟设置。
- Windows Edge 本地 Web release 实际导入平板保存的 `.bfly`，可见压力粗细；鼠标新增一笔并通过界面导出。原文件 94 个元素，导出 95 个元素；JSON 数字规范化后原有 94 个元素逐项完全一致（含坐标、压力和笔刷属性）。证据与测试文件只存忽略目录，不提交用户笔迹。
- 此记录时新版 APK 构建中，平板休眠，尚未完成新版真机撤销保存与电脑文件回传重开。未配置 WebDAV/Nextcloud，不宣称自动同步通过。

### 保存修复 APK 真机复验通过

- 代码提交 `a7604a939` 已推送并核对远端 OID；ARMv7 APK SHA256 `F656508823C2880B5143B144203B01EC0A96535C02E8876D3D76001CE2DE8CBC`，原位安装成功。构建参数 `-SkipPubGet -NativeInkExperiment -NativeInkDiagnostics`，真机使用 `commit-only`；普通 release 编译配置不等于已签发正式发行包。
- Edge 界面导出文件 SHA256 `2532E86A96FE702E2AD19B6873132C71DCD1337C233EF3DB6ADEA4B3B8D58781`，浏览器刷新后从本地文件列表重开，电脑新增 V 形笔画仍在。
- 该文件复制至平板独立测试名称后经 `/local/…bfly` 冷启动打开；平移画布可见电脑新增 V 形线及原有压力笔迹。用 ADB 触摸拖动新增一条受控测试线，保存后共 96 个元素；这条合成触摸只用于保存与历史测试，不替代实体笔延迟验收。
- 真机点击 Undo→Save 后文件为 95 个元素，前 95 个元素与新增前一致；Redo→Save 恢复 96 个元素，与 Undo 前所有元素逐项一致，包括 10,106 个压力采样。随后 force-stop 并重开，文件仍为相同 96 个元素，UI 为 Saved。缩放与平移 smoke 已执行，最终恢复 100% 并选回笔工具。
- 本次冷启动 PID 18568，crash buffer 无条目。格式检查 3 文件无改动。用户对原实时压力实现的实体书写验收仍有效，但本次没有重新要求用户重复实笔；不能据此声称所有旋转、笔宽/缩放匹配、长时间残影、导出格式或联网同步均已验收。
