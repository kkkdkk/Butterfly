...
- [ ] P8：73c37e04d 已实现并通过定向测试、构建及真机安装；用户反馈仍然很慢。全量检查尚待补齐，低延迟验收未通过。
- [ ] P9：独立探针已构建并装机，用户确认带 brush 初始化的首版书写流畅；移除该调用后不能写。be464f52e 恢复版已重新装机并启动，待复验；生命周期、输入完整性与交接尚未通过。见 PROBE-VERIFICATION.md。
- [ ] P10：仅在 P9 Go 时执行。
